<footer class="bg-[#966727] font-lato min-lg:px-25 md:px-10 max-sm:px-10">
    <div class="mx-auto w-full max-w-screen-xl py-8">
        <div class="md:flex md:justify-start">
        <div class="mb-6 md:mb-0">
            <a href="/" class="flex items-center gap-2">
                <img src="{{ asset('storage/image/cikopilogo.png')}}" class="w-14 me-3" alt="Cikopi Mang Eko Logo" />
                <span class="self-center text-2xl font-regular whitespace-nowrap dark:text-white">Cikopi Mang Eko</span>
            </a>
        </div>
    </div>
    <div class="mx-auto w-full max-w-screen-xl min-lg:pl-30 md:pl-0 sm:pl-10 py-8">
        <div class="gap-x-15 gap-y-10 flex flex-wrap">
            <div class="w-75 ">
                <h2 class="mb-6 text-sm font-semibold text-gray-900 uppercase dark:text-white">Store</h2>
                <ul class="text-white font-medium">
                    <li class="mb-4 flex gap-4">
                        <img class="h-[32px] pt-1" src="{{ asset('storage/image/location.png')}}" alt="">
                        <a href="https://maps.app.goo.gl/zqsu35aAvGHirUYw8" class="hover:underline lg:w-60 md:w-auto" target="_blank" rel="noopener noreferrer">Jl. Golf  Dalam No.2 Blok G, Cisaranten Bina Harapan, Kec Arcamanik, Kota Bandung, Jawa Barat 40294.</a>
                    </li>
                </ul>
            </div>
            <div class="w-75">
                <h2 class="mb-6 text-sm font-semibold text-gray-900 uppercase dark:text-white">Contact</h2>
                <ul class="text-white font-medium">
                    <li class="mb-4 flex gap-4 place-items-center">
                        <img class="h-[32px] pt-1" src="{{ asset('storage/image/whatsapp.png')}}" alt="">
                        <a href="https://api.whatsapp.com/send/?phone=6281350508811&text&type=phone_number&app_absent=0" class="hover:underline" target="_blank" rel="noopener noreferrer">+62 813 - 5050 - 8811</a>
                    </li>
                </ul>
            </div>
            <div class="w-75 ">
                <h2 class="mb-6 text-sm font-semibold text-gray-900 uppercase dark:text-white">Social Media</h2>
                <ul class="text-white font-medium">
                    <li class="mb-4 flex gap-4 place-items-center">
                        <img class="h-[32px] pt-1" src="{{ asset('storage/image/instagram1.png')}}" alt="">
                        <a href="https://www.instagram.com/cikopimangeko?igsh=MWJlNTNmaHBoeHJ4dQ==" class="hover:underline" target="_blank" rel="noopener noreferrer">@cikopimangeko</a>
                    </li>
                </ul>
            </div> 
            <div class="w-75 ">
                <h2 class="mb-6 text-sm font-semibold text-gray-900 uppercase dark:text-white">Operational Hours</h2>
                <ul class="text-white font-medium">
                    <li class="mb-4 flex gap-4">
                        <img class="h-[32px] pt-1" src="{{ asset('storage/image/time.png')}}" alt="">
                        <div class="grid grid-cols-1">
                            <p class="">Senin - Minggu</p>
                            <p class="">10.00 - 18.00 WIB</p>
                        </div>
                    </li>
                </ul>
            </div> 
            <div class="w-75 ">
                <h2 class="mb-6 text-sm font-semibold text-gray-900 uppercase dark:text-white">Online Shop</h2>
                <ul class="text-white font-medium">
                    <li class="mb-4 flex gap-4 place-items-center">
                        <img class="h-[32px] pt-1" src="{{ asset('storage/image/tokopedia1.png')}}" alt="">
                        <a href="https://tk.tokopedia.com/ZSBm1JbXe/" class="hover:underline" target="_blank" rel="noopener noreferrer">cikopimangeko</a>
                    </li>
                </ul>
            </div> 
        </div>
    </div>
    {{-- <hr class="my-6 border-gray-200 sm:mx-auto dark:border-gray-700 lg:my-8" /> --}}
    <div class="sm:flex sm:items-center sm:justify-between">
        <span class="text-sm text-white sm:text-center">©2025<a href="https://flowbite.com/" class="hover:underline">cikopimangeko</a>
        </span>
    </div>
    </div>
</footer>